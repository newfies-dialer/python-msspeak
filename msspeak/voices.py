#
# Retrieved from get_listvoice.py using API
#
...
AZURE_VOICES = [
    ...
    {
        "Name": "Microsoft Server Speech Text to Speech Voice (es-ES, HelenaNeural)",
        "ShortName": "es-ES-HelenaNeural",
        "Gender": "Female",
        "Locale": "es-ES"
    },
    {
        "Name": "Microsoft Server Speech Text to Speech Voice (es-MX, HildaNeural)",
        "ShortName": "es-MX-HildaNeural",
        "Gender": "Female",
        "Locale": "es-MX"
    },
    {
...
def find_voice(locale, gender, neural=True):
    ...
    """
    voice_no_neural = False
    i = 0
    # try to find the neural
    for voice in AZURE_VOICES:
        i = i + 1
        if voice['Locale'] == locale and voice['Gender'] == gender and voice['ShortName'][-6:] == 'Neural' and neural == True:
            return voice
        elif voice['Locale'] == locale and voice['Gender'] == gender and voice['ShortName'][-6:] != 'Neural':
            voice_no_neural = voice

    return voice_no_neural


# voice = find_voice(locale='en-US', gender='Male', neural=False)
